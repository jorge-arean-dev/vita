...
function extractSkillsFromPdfText(pdfText) {
  const skillsFound = [];
  ...
  techKeywords.forEach((tech)=>{
    if (lowerCaseText.includes(tech.toLowerCase())) {
      ...
      const relevantSentences = sentences.filter((sentence)=>sentence.toLowerCase().includes(tech.toLowerCase()) && sentence.trim().length > 20);
      ...
      if (existingSkill) {
        ...
        skillsFound.push({
          ...
          years: skillYears,
          evidence: relevantSentences.length > 0 ? relevantSentences.map((s)=>s.substring(0, 100) + '...') : [
      ...
      if (aboutText.includes(tech)) {
        ...
        if (existingSkill) {
          ...
          skillsFound.push({
            ...
            years: 1,
            evidence: [
              'Mentioned in resume about section'
            ],
            specificEvidence: specificEvidence
          });
        }
...
function generateRecruiterFeedback(requirement, matches, skillType, score, status) {
  ...
  if (primaryMatch.matchType === 'exact') {
    ...
    if (hasEvidence) {
      ...
      if (bestEvidence) {
        const linkedInLocation = bestEvidence.company && bestEvidence.role ? `their resume shows this during their role as ${bestEvidence.role} at ${bestEvidence.company}` : `evidenced in their resume ${bestEvidence.source}`;
    ...
    if (hasEvidence) {
      ...
      if (implementationEvidence) {
        const linkedInRef = implementationEvidence.company && implementationEvidence.role ? `their resume during their time as ${implementationEvidence.role} at ${implementationEvidence.company}` : `their resume ${implementationEvidence.source}`;
    ...
    if (hasEvidence) {
      ...
      const linkedInReference = bestEvidence.company && bestEvidence.role ? `their resume shows this experience as ${bestEvidence.role} at ${bestEvidence.company}` : `evidenced in their resume ${bestEvidence.source}`;
  ...
  if (additionalMatches.length > 0) {
    const additionalSkills = additionalMatches.slice(0, 2).map((m)=>{
      ...
      if (m.specificEvidence && m.specificEvidence.length > 0) {
        ...
        const linkedInContext = evidence.company ? ` (from their work at ${evidence.company})` : evidence.context ? ` (${evidence.context})` : ` (from resume)`;
