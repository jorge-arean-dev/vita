...
/**
 * Loading component for Interview Analysis
 */
...
/**
 * Server component that handles Interview Analysis page routing and data fetching
 */
export default async function InterviewAnalysisPage({ params }: InterviewAnalysisPageProps) {
  ...
  return (
    <Suspense fallback={<InterviewAnalysisSkeleton />}>
      <InterviewAnalysisContainer jobId={resolvedParams.id} />
    </Suspense>
  )
}

/**
 * Container component that fetches job data and renders the Interview Analysis
 */
async function InterviewAnalysisContainer({ jobId }: { jobId: string }) {
  // Fetch job data
