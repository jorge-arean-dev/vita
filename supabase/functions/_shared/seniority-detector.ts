...
// Detect seniority level from candidate profile
 ...
 */ const TITLE_PATTERNS = [
  ...
  {
    pattern: /\b(ceo|cto|cfo|coo|cpo|ciso|cmo|chief|c-level|president|vp|vice president|evp|svp)\b/i,
    level: 'executive',
    confidence: 'high'
  },
  {
    pattern: /\b(executive|director|head of|managing director)\b/i,
    level: 'executive',
    confidence: 'medium'
  },
  // Lead patterns
  {
    pattern: /\b(lead|team lead|tech lead|technical lead|engineering lead|principal|staff|architect|manager|supervisor)\b/i,
    level: 'lead',
    confidence: 'high'
  },
  {
    pattern: /\b(senior manager|senior architect)\b/i,
    level: 'lead',
    confidence: 'high'
  },
  // Senior patterns
  {
    pattern: /\b(senior|sr\.?)\s+((?!manager|director|vp|president|lead|architect).)*$/i,
    level: 'senior',
    confidence: 'high'
  },
  {
    pattern: /\b(expert|specialist|consultant|advisor)\b/i,
    level: 'senior',
    confidence: 'medium'
  },
  // Mid-level patterns
  {
    pattern: /\b(mid-level|mid level|mid-senior|mid senior|intermediate)\b/i,
    level: 'mid',
    confidence: 'high'
  },
  {
    pattern: /^(?!.*\b(junior|jr|senior|sr|lead|principal|staff|director|manager|executive|chief|head|vp|president)\b)[a-z\s]+\s+(developer|engineer|analyst|designer|scientist|specialist)$/i,
    level: 'mid',
    confidence: 'medium'
  },
  // Junior patterns
  {
    pattern: /\b(junior|jr\.?|entry level|entry-level|associate|intern|trainee|apprentice|graduate)\b/i,
    level: 'junior',
    confidence: 'high'
  },
  {
    pattern: /\b(assistant|coordinator)\b(?!.*\b(senior|lead|manager|director)\b)/i,
    level: 'junior',
    confidence: 'medium'
  }
];
 ...
 */ export function detectSeniorityFromTitle(title) {
 ...
 */ export function detectSeniorityFromExperience(yearsOfExperience) {
 ...
 */ export function detectCandidateSeniority(candidate) {
  ...
  if (candidate.titles && candidate.titles.length > 0) {
    const titleLevels = candidate.titles.map((title)=>detectSeniorityFromTitle(title)).filter((result)=>result.level !== null);
  ...
  if (candidate.roles && candidate.roles.length > 0) {
    const roleSeniorities = candidate.roles.map((role)=>detectSeniorityFromTitle(role.title)).filter((result)=>result.level !== null);
    if (roleSeniorities.length > 0) {
      ...
      if (latestLevel) {
        const hasProgression = roleSeniorities.length > 1 && levelOrder.indexOf(latestLevel) > levelOrder.indexOf(roleSeniorities[0].level);
        results.push({
          level: latestLevel,
