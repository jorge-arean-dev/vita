...
// Core matching engine for analyzing candidates against job requirements
 ...
 */ import { matchAllRequirements, extractSupplementarySkills } from './skill-matcher.ts';
 import { calculateFinalScore, createRequirementScore } from './score-calculator.ts';
 /**
  * Main analysis function - orchestrates the entire matching process
  */ export async function analyzeCandidate(candidate, job, rawData, options) {
   ...
   const structuredMatches = await matchAllRequirements(job.requirements, candidate.skills, {
   ...
   const gapRequirements = structuredMatches.filter((match)=>match.score < 70).filter((match)=>match.requirement && match.requirement.name) // Defensive check
   ...
   if (rawData && gapRequirements.length > 0) {
     ...
     if (rawData.resumeText) {
       const resumeSkills = await extractSupplementarySkills(rawData.resumeText, gapRequirements);
     ...
     if (rawData.linkedInProfile) {
       ...
       const linkedInSkills = await extractSupplementarySkills(linkedInText, gapRequirements);
   ...
   if (supplementarySkills.length > 0) {
     ...
     const gapReqs = job.requirements.filter((req)=>req && req.name && gapRequirements.includes(req.name));
     const supplementaryMatches = await matchAllRequirements(gapReqs, supplementarySkills, {
   ...
   const mandatoryMatches = finalMatches.filter((m)=>m.requirement && m.requirement.importance === 'mandatory');
   const optionalMatches = finalMatches.filter((m)=>m.requirement && m.requirement.importance === 'optional');
   const mandatoryScores = mandatoryMatches.filter((match)=>match.requirement && match.requirement.name) // Defensive check
   .map((match)=>createRequirementScore({
       id: match.requirement.id,
       name: match.requirement.name
     }, match.score, match.bestMatch ? [
       match.explanation
     ] : [], match.bestMatch?.confidence ?? 0));
   const optionalScores = optionalMatches.filter((match)=>match.requirement && match.requirement.name) // Defensive check
   .map((match)=>createRequirementScore({
       id: match.requirement.id,
       name: match.requirement.name
     }, match.score, match.bestMatch ? [
   ...
   return {
     ...
     metadata: {
       ...
       processingStrategy: supplementarySkills.length > 0 ? 'structured-primary-raw-supplementary' : 'structured-only',
       skillsUsed: {
  ...
  */ function mergeMatches(structuredMatches, supplementaryMatches) {
  ...
  */ function prepareDetailedMatches(matches) {
   return matches.map((match)=>({
       requirement: match.requirement,
       score: match.score,
       category: match.score >= 80 ? 'fit' : match.score >= 60 ? 'developing' : match.score >= 30 ? 'weak' : 'missing',
       matchedSkills: match.matches.map((m)=>m.candidateSkill),
       evidence: match.bestMatch ? [
         match.explanation
       ] : [],
  ...
  */ export function processLinkedInData(linkedInProfile) {
   // Extract basic info
  ...
  */ export function processPDFData(resumeText, extractedSkills, candidateInfo) {
   return {
     candidate: {
  ...
  */ export function processExistingCandidate(candidateData, candidateSkills) {
