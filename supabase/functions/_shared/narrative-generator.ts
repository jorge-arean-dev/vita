...
// Generate narrative explanations for match analysis
// Used by: match-analysis functions
/**
 ...
 */ export async function generateNarrativeOutputs(analysisResult, jobTitle, options = {}) {
    const { focusOnProficiency = true, includeInterviewStrategy = true, maxStrengths = 5, maxGaps = 5, maxRecommendations = 4 } = options;
    // Generate requirement-level feedback first
    const requirementEvaluations = await generateRequirementFeedback([
      ...analysisResult.detailedMatches.mandatory,
      ...analysisResult.detailedMatches.optional
    ], focusOnProficiency);
    // Generate summary strengths and gaps
    const { strengths, gaps } = await generateStrengthsAndGaps(analysisResult, jobTitle, maxStrengths, maxGaps);
    // Generate recruiter recommendations
    const recruiterRecommendations = await generateRecruiterRecommendations(analysisResult, jobTitle, includeInterviewStrategy, maxRecommendations);
    // Generate overall feedback
    const overallFeedback = await generateOverallFeedback(analysisResult, jobTitle, strengths, gaps);
    return {
      summary: {
        strengths,
        gaps
   ...
   */ async function generateRequirementFeedback(matches, focusOnProficiency) {
   ...
   */ async function generateStrengthsAndGaps(analysisResult, jobTitle, maxStrengths, maxGaps) {
    const allMatches = [
      ...analysisResult.detailedMatches.mandatory,
      ...analysisResult.detailedMatches.optional
    ];
    // Identify strengths (fit matches only - developing/weak are gaps)
    const fitMatches = allMatches.filter((m)=>m.category === 'fit').sort((a, b)=>b.score - a.score).slice(0, maxStrengths);
    ...
    const gapMatches = allMatches.filter((m)=>(m.category === 'developing' || m.category === 'weak' || m.category === 'missing') && m.requirement.importance === 'mandatory').slice(0, maxGaps);
   ...
   */ async function generateRecruiterRecommendations(analysisResult, jobTitle, includeInterviewStrategy, maxRecommendations) {
    const mandatoryGaps = analysisResult.detailedMatches.mandatory.filter((m)=>m.category === 'weak' || m.category === 'missing');
    const overallScore = analysisResult.overallScore.totalScore;
    const category = analysisResult.overallScore.category;
    let interviewStrategy = [];
    let otherOptions = [];
    if (includeInterviewStrategy) {
      ...
      if (mandatoryGaps.length > 0) {
        ...
        interviewStrategy.push(`Focus interview on assessing practical experience with: ${gapSkills.join(', ')}`);
      ...
      const fitSkills = analysisResult.detailedMatches.mandatory.filter((m)=>m.category === 'fit').map((m)=>m.requirement.name);
   ...
   */ async function generateOverallFeedback(analysisResult, jobTitle, strengths, gaps) {
    ...
    const mandatoryMet = analysisResult.detailedMatches.mandatory.filter((m)=>m.category === 'fit').length;
    let feedback = '';
    // Score-based opening
    if (category === 'fit') {
   ...
   */ function generateBasicFeedback(skill, category, isMandatory) {
    if (category === 'fit') {
   ...
   */ export function generateVariedContent(templates, context) {
