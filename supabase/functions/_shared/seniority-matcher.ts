...
import type { SeniorityLevel } from './seniority-detector.ts'

export interface SeniorityMatchResult {
  ...
  isUnderqualified: boolean
}

/**
 * Seniority level hierarchy for comparison
 */
const SENIORITY_HIERARCHY: Record<SeniorityLevel, number> = {
  'junior': 1,
  'mid': 2,
  'senior': 3,
  'lead': 4,
 ...
 */
function calculateSeniorityGap(
 ...
 */
function calculateScore(gap: number): number {
 ...
 */
export function adjustCategoryForSeniority(
  currentCategory: 'fit' | 'developing' | 'weak' | 'missing',
  seniorityScore: number
): 'fit' | 'developing' | 'weak' | 'missing' {
 ...
 */
export function generateSeniorityRecommendations(result: SeniorityMatchResult): string[] {
  const recommendations: string[] = []
  
  if (result.isOverqualified) {
    recommendations.push(
      'Discuss career motivations and long-term goals to understand interest in this level',
      'Clarify growth opportunities and timeline for advancement',
      'Verify compensation expectations align with the role level'
    )
  } else if (result.isUnderqualified) {
    const gap = result.required && result.candidate 
      ? SENIORITY_HIERARCHY[result.required] - SENIORITY_HIERARCHY[result.candidate]
      : 0
    
    if (gap === 1) {
      recommendations.push(
        'Assess readiness to step up to higher responsibilities',
        'Discuss support structure and mentorship available',
        'Evaluate specific experiences that demonstrate next-level capabilities'
      )
    } else {
      recommendations.push(
        'Consider for future talent pipeline',
        'Explore other open positions at appropriate level',
        'Provide feedback on experience needed for target role'
      )
    }
  }
