...
serve(async (req)=>{
  ...
  try {
    ...
    // Parse request body
    const { interview_id, performance_level } = await req.json();
    if (!interview_id || !performance_level) {
      return new Response(JSON.stringify({
        error: 'Missing required fields: interview_id and performance_level'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: interview, error: interviewError } = await supabaseClient.from('interviews').select(`
        id,
        status,
        jobs!inner (
          title,
    ...
    if (interview.status !== 'created') {
      return new Response(JSON.stringify({
        error: 'Interview simulation only allowed for interviews in "created" status'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const jobDetails = interview.jobs;
...
async function generateTranscript(jobDetails, performanceLevel) {
