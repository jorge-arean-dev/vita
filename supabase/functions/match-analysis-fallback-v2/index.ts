 ...
 */ import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
 ...
 serve(async (req)=>{
   ...
   try {
     ...
     const titleSkills = candidate.skills.filter((skill)=>skill.name && typeof skill.name === 'string').filter((skill)=>/(?:lead|manager|director|senior|junior|principal|staff|architect|engineer|developer|analyst|specialist|coordinator|assistant|associate)/i.test(skill.name));
     const extractedTitles = titleSkills.map((skill)=>skill.name).slice(0, 5) // Top 5 relevant titles
     ;
     const roleSkills = candidate.skills.filter((skill)=>skill.source === 'role' || /(?:lead|manager|director|senior|junior|principal|staff|architect|engineer|developer|analyst|specialist|coordinator|assistant|associate)/i.test(skill.name || ''));
     ...
     const response = formatFallbackResponse(analysisResult, candidate, candidateData, narrativeOutputs, seniorityMatchResult);
  ...
  */ function formatFallbackResponse(analysisResult, candidate, originalCandidateData, narrativeOutputs, seniorityMatchResult) {
